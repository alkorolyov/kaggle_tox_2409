...
from src.descriptors import get_md_descriptors
...
def get_mord_features(smiles):
    ...
    res = Parallel(n_jobs=mp.cpu_count())(delayed(calc_md)(mol) for mol in molecules)

    return pd.DataFrame(np.stack(res), index=smiles.index, columns=md_cols)
...
def get_fingerprints(
    ...
    if morgan_fps:
        fingerprints.append(mem.cache(get_morgan_fingerprints)(smiles))
    if avalon_fps:
        fingerprints.append(mem.cache(get_avalon_fingerprints)(smiles))
    if erg_fps:
        fingerprints.append(mem.cache(get_erg_fingerprints)(smiles))
    if rdkit_feats:
        fingerprints.append(mem.cache(get_rdkit_features)(smiles))
    if mord_feats:
